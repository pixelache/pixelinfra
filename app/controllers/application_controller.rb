class ApplicationController < ActionController::Base
  ...
  def determine_site
    if Rails.env.development?
      ...
      if request.host =~ /^livingspaces/
        ...
        @site = Subsite.find_by(name: 'empathy') 
      elsif   request.host =~ /^festival2017/    || request.host =~ /^festival/
  ...
  def configure_permitted_parameters
    added_attrs = [:username, :email, :password, :password_confirmation, :avatar, :remember_created_at, :sign_in_count, :slug, :remember_me]
